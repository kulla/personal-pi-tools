# personal-pi-tools

My personal [pi](https://github.com/earendil-works/pi) tools package.

## Install

```bash
pi install git:github.com/kulla/personal-pi-tools
```

## Extensions

### /commit

Source: `extensions/commit/index.ts`

Generates a conventional commit message from the current pi session and git diff, opens it in an editor for review, then stages all changes and commits with the accepted message. It tries user prompts first, then user prompts plus AI result messages, and records durable info/warning/error log entries for each prompt sent to the LLM.

Usage:

```text
/commit
```

## Utilities

### `utils/logging.ts`

Reusable log-entry renderer/helper for extensions.

Usage:

```ts
import { createLogger } from "./utils/logging.ts";

const logger = createLogger(pi, "my-log");
logger.log("Hello", "info");
```
